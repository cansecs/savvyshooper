...
export default class CameraScreen extends React.Component {
  ...
  componentDidMount() {
      /*FileSystem.getInfoAsync(AppFolder)
      .then(result=>{
      console.log("Checking",AppFolder,result);
      if ( result && !result.exists){
        FileSystem.makeDirectoryAsync(AppFolder).catch(e => {
  ...
  onPictureSaved = async photo => {
    console.log("Pciture has taken",photo);
    const {status } = await Permissions.askAsync(Permissions.CAMERA_ROLL);
   console.log("Access to cameraroll is granted",status);
    this.setState({storagePermission: status === 'granted' })

    if (true){//this.state.storagePermission){
      console.log("Saving result",photo.uri);
    let saveResult = await  CameraRoll.saveToCameraRoll(photo.uri,'photo');
    /*await FileSystem.moveAsync({
      from: photo.uri,
      to: `${AppFolder}/${Date.now()}.jpg`,
    });*/
    console.log("Photo saved",saveResult);
    this.setState({ newPhotos: saveResult });
    this.props.navigation.navigate('Home')
  }else{
    Alert.alert("You didn't allow us to save picture!")
  }
  }

  onBarCodeRead = code => {
    Alert.alert("Found:"+JSON.stringify(code));
    this.takePicture();
  };
  ...
  collectPictureSizes = async () => {
    if (this.camera && typeof this.camera.getAvailablePictureSizeAsync == 'function' ) {
